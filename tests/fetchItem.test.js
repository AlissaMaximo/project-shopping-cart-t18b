...
describe('2 - Teste a função fecthItem', () => {
  ...
  it('Teste se, ao chamar a função fetchItem com o argumento do item "MLB1615760527", a função fetch utiliza o endpoint "https://api.mercadolibre.com/items/MLB1615760527"', () => {
    const myItem = 'https://api.mercadolibre.com/items/MLB1615760527';
    fetchItem('MLB1615760527');
    expect(fetch).toBeCalledWith(myItem);
  });
  ...
  it('Teste se, ao chamar a função fetchItem sem argumento, retorna um erro com a mensagem: You must provide an url. ', async () => {
    fetchItem().catch((error) => expect(error).toEqual(new Error('You must provide an url')));
  });
